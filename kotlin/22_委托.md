...
CustomPrinter 的 by 子句表示将会在 CustomPrinter 中存储  printer 变量，并且编译器将隐式生成所有 Printer 接口的抽象方法，并将这些方法的调用操作转发给 printer 
...
Kotlin 支持通过委托属性将对一个属性的访问操作委托给另外对象来完成，对应的语法是

```kotlin
val/var <属性名>: <类型> by <表达式>
```

**属性的委托**不必实现任何的接口，但需要提供一个  **getValue()**  方法与  **setValue()** （对于 var  属性），对一个属性的 get 和 set 操作会被委托给**属性的委托**的 **getValue()** 与 **setValue()** 方法
