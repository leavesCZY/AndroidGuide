> 国庆假期想着闲着也是闲着，就想着来深入了解下几个常用的开源库，看下其实现原理和源码，进行总结并输出成文章。初定的目标是 **EventBus、ARouter、LeakCanary、Retrofit、Glide、Coil、OkHttp** 等几个。目前已经完成了六篇关于 EventBus、ARouter、LeakCanary 的文章，本篇是第七篇，来对 Retrofit 进行讲解😂😂希望对你有所帮助
